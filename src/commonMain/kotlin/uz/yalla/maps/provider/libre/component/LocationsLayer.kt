...
private class LocationMarkerPainter(
    ...
    private val markerSize = with(density) { MarkerDimens.MarkerSize.toPx() }
    private val borderWidth = with(density) { MarkerDimens.BorderWidth.toPx() }
    private val badgeHeight = with(density) { MarkerDimens.BadgeHeight.toPx() }
    private val badgePadding = with(density) { MarkerDimens.BadgePadding.toPx() }
    ...
    private fun DrawScope.drawMarker(
        ...
        drawCircle(color = MarkerColors.MarkerFill, radius = markerSize / 2 - borderWidth, center = center)
    }
}
